...
def test_generate_sine_input_parameters():
    ...
    rms, f, t, fs = 1, 1, 1, 1000

    a = transix.generate_sine(rms=rms, f=f, t=t, fs=fs)

    # Magnitude check
    assert np.allclose(np.max(a)/np.sqrt(2), rms, atol=1e-7)
...
def test_generate_abc_input_parameters():
    ...
    r,y,b = transix.generate_abc(rms=rms, f=f, t=t, fs=fs)

    # Magnitude check
    assert np.allclose(np.max(r)/np.sqrt(2), rms, atol=1e-7)
    assert np.allclose(np.max(y)/np.sqrt(2), rms, atol=1e-7)
    assert np.allclose(np.max(b)/np.sqrt(2), rms, atol=1e-7)
...
def test_generate_sine_output_parameters():
    ...
    rms, f, t, fs = 1, 1, 1, 5000

    a = transix.generate_sine(rms=rms, f=f, t=t, fs=fs)

    # Output magnitude check
    assert np.isclose(a[0], rms * np.sqrt(2) * np.sin(2*np.pi*f*0), atol=1e-7)
    assert np.isclose(a[-1], rms * np.sqrt(2) * np.sin(2*np.pi*f*t), atol=1e-7)
    assert np.isclose(a[0], a[-1], atol=1e-7)

    # Output peak value check
    expected_peak = np.sqrt(2) * rms
...
def test_generate_abc_output_parameters():
    ...
    # Output magnitude check
    assert np.isclose(
        r[0], rms * np.sqrt(2) * np.sin(2*np.pi*f*0), atol=1e-7)
    assert np.isclose(
        y[0], rms * np.sqrt(2) * np.sin(2*np.pi*f*0 + np.deg2rad(-120)), atol=1e-7)
    assert np.isclose(
        b[0], rms * np.sqrt(2) * np.sin(2*np.pi*f*0 + np.deg2rad(-240)), atol=1e-7)
...
def test_generate_sine_phase_shift():
    ...
    phase_shift = 90
    a = transix.generate_sine(rms=rms, f=f, t=t, fs=fs, phase_shift=phase_shift)

    assert np.isclose(a[0], rms * np.sqrt(2), atol=1e-7)
...
def test_generate_abc_phase_shift():
    ...
    for shift in [0,90,180,270,360]:
        r, y, b = transix.generate_abc(
            rms=rms, f=f, t=t, fs=fs, phase_shift=shift)

        a_ref = transix.generate_sine(
            rms=rms, f=f, t=t, fs=fs, phase_shift=0 + shift)
        b_ref = transix.generate_sine(
            rms=rms, f=f, t=t, fs=fs, phase_shift=-120 + shift)
        c_ref = transix.generate_sine(
            rms=rms, f=f, t=t, fs=fs, phase_shift=-240 + shift)
