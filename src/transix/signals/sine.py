...
def generate_sine(rms, f, t, fs, phase_shift=0.0):
    r"""
    ...
    return np.sqrt(2) * rms * np.sin(w * time_array + phi)




def generate_abc(rms, f, t, fs, phase_shift=0.0):
    r"""
    ...
    a = np.sqrt(2) * rms * np.sin(w*time_array + phi)
    b = np.sqrt(2) * rms * np.sin(w*time_array + np.deg2rad(-120) + phi)
    c = np.sqrt(2) * rms * np.sin(w*time_array + np.deg2rad(-240) + phi)
